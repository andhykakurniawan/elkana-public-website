          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {news.map((item, index) => (
              <article key={item.title} className="overflow-hidden rounded-2xl border border-purple-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                <img src={gallery[index % gallery.length].src} alt={item.title} className="h-52 w-full object-cover" />
                <div className="p-6">
                  <div className="mb-4 flex items-center gap-3">
                    <span className="rounded-xl bg-purple-600 px-3 py-2 text-center text-sm font-black text-white">{item.category}</span>
